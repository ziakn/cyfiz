...
export async function GET() {
  try {
    const [experience, skills, projects, education, certifications, researchPapers, socialLinks, stats] = await Promise.all([
      ...
      query<ResearchPaperRow[]>("SELECT id, title, tag, source, DATE_FORMAT(date, '%M %d, %Y') as date, read_time as readTime FROM research_summaries WHERE status = 1 ORDER BY research_summaries.date DESC, id DESC LIMIT 3"),
