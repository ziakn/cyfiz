...
export default function PortfolioList({ initialExperience, initialSkills, initialProjects }: PortfolioListProps) {
  ...
  const handleStatusToggle = async (table: string, id: number, newStatus: number) => {
    const result = await toggleStatusAction(table, id, newStatus, ["/admin/portfolio", "/profile"]);
                  ...
                  <tr key={exp.id} className="transition-colors hover:bg-[#F9FAFB]">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button 
                          onClick={() => setModal({ open: true, type: 'experience', action: 'edit', data: exp })}
                        ...
                        <button 
                          onClick={() => setDeleteModal({ open: true, type: 'experience', id: exp.id, title: exp.company })}
            ...
            <form action={async (formData) => {
              ...
              {modal.type === 'experience' && (
                <>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-[#3A3541] opacity-[0.6]">Location</label>
                    <input name="location" required defaultValue={modal.data?.location} className="w-full rounded-md border border-[#3A3541] border-opacity-[0.22] px-3 py-2 text-sm outline-none focus:border-[#9155FD]" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-[#3A3541] opacity-[0.6]">Bullets</label>
                    <textarea name="bullets" required rows={4} defaultValue={modal.data?.bullets} className="w-full rounded-md border border-[#3A3541] border-opacity-[0.22] px-3 py-2 text-sm outline-none focus:border-[#9155FD]" />
                  </div>
                </>
              )}
              {modal.type === 'project' && (
                <>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-[#3A3541] opacity-[0.6]">Description</label>
                    <textarea name="description" required rows={3} defaultValue={modal.data?.description} className="w-full rounded-md border border-[#3A3541] border-opacity-[0.22] px-3 py-2 text-sm outline-none focus:border-[#9155FD]" />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-[#3A3541] opacity-[0.6]">Link</label>
                    <input name="href" defaultValue={modal.data?.href} placeholder="#" className="w-full rounded-md border border-[#3A3541] border-opacity-[0.22] px-3 py-2 text-sm outline-none focus:border-[#9155FD]" />
                  </div>
                </>
              )}
              {modal.type === 'skill' && (
