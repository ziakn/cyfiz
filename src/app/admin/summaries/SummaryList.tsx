    ...
    <div className="space-y-6">
                ...
                <tr key={summary.id} className="transition-colors hover:bg-[#F9FAFB]">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button 
                        onClick={() => {
                          setEditExcerpt(summary.excerpt);
                          setEditModal({ open: true, summary });
                        }}
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#3A3541] bg-opacity-50 p-4 transition-opacity">
          <div className="max-h-[calc(100vh-2rem)] w-full max-w-2xl scale-100 overflow-y-auto rounded-lg bg-white p-8 shadow-[0_12px_40px_rgba(58,53,65,0.2)] transition-transform">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-bold text-[#3A3541] opacity-[0.87]">Add New Summary</h3>
              <button onClick={() => {
                setAddExcerpt("");
                setAddModalOpen(false);
              }} className="text-[#3A3541] opacity-[0.54] hover:opacity-100">
          ...
          <div className="max-h-[calc(100vh-2rem)] w-full max-w-2xl scale-100 overflow-y-auto rounded-lg bg-white p-8 shadow-[0_12px_40px_rgba(58,53,65,0.2)] transition-transform">
            ...
            <form action={async (formData) => {
              ...
              <div className="space-y-1">
                ...
                <input type="date" name="date" required defaultValue={toDateValue(editModal.summary.date)} className="w-full rounded-md border border-[#3A3541] border-opacity-[0.22] px-3 py-2 text-sm outline-none focus:border-[#9155FD]" />
              </div>
