...
export default function TeamList({ initialTeam }: { initialTeam: TeamMember[] }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; memberId: number | null; name: string }>({
    open: false,
    memberId: null,
    name: "",
  });
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    const result = await actions.deleteTeamMemberAction(deleteModal.memberId);
    setIsDeleting(false);
    if (result.success) setDeleteModal({ open: false, memberId: null, name: "" });
            ...
            <thead className="bg-[#F9FAFB] border-b border-[#3A3541] border-opacity-[0.12]">
              <tr>
                <th className="px-6 py-4">
                 ...
                 <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#3A3541] opacity-[0.38]">Avatar</th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#3A3541] opacity-[0.38]">Member</th>
                ...
                <tr key={member.id} className="transition-colors hover:bg-[#F9FAFB]">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-[#3A3541] opacity-[0.87]">{member.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <StatusToggle 
                        ...
                        onToggle={(newStatus) => handleStatusToggle(member.id, newStatus)} 
                      />
                      <span className={`text-[10px] font-bold uppercase tracking-wider ${
                        member.status === 1 ? "text-[#56CA00]" : "text-gray-400"
                      }`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button 
                        onClick={() => setModal({ open: true, action: 'edit', data: member })}
          ...
          <div className="w-full max-w-lg scale-100 rounded-lg bg-white p-8 shadow-[0_12px_40px_rgba(58,53,65,0.2)] transition-transform">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-bold text-[#3A3541] opacity-[0.87]">
                {modal.action === 'add' ? 'Add New' : 'Edit'} Member
              </h3>
            ...
            <form action={async (formData) => {
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-[#3A3541] opacity-[0.6]">Name</label>
